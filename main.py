...
#    white = (0, 0, 255 )
    ...
    while instructions:
        ...
        gameFunctions.print_text(myfont, 10, 400, "But before playing we need to take you to some other screens.", white, screen)
    ...
    while intro_player1:
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                else:  # if user tries to capitalize name, this will ignore it
                    if event.key != K_RSHIFT and event.key != K_LSHIFT:
                        player1_name += pygame.key.name(event.key)
            screen.fill(black)

        # this prints the name in capitals as the user writes it. Pygame updates the screen fast enough.
        screen.blit(win.render(player1_name.upper(), 1, green), (0, 250))
        pygame.display.update()
        ...
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                if event.key == K_RETURN or event.key == K_KP_ENTER:
                    ...
                    if event.key != K_RSHIFT and event.key != K_LSHIFT:
                        player2_name += pygame.key.name(event.key)
            screen.fill(black)

        # this prints the name in capitals as the user writes it. Pygame updates the screen fast enough.
        screen.blit(win.render(player2_name.upper(), 1, green), (0, 250))
        pygame.display.update()
